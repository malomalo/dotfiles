...
syn region ledgerTransaction start=/^[[:digit:]~]/ skip=/^\s/ end=/^/
    \ keepend fold transparent contains=ledgerTransactionDate,ledgerMetadata,ledgerPosting
syn match ledgerTransactionDate /^\d\S\+/ contained
syn match ledgerPosting /^\s\+[^[:blank:];][^;]*\ze\%($\|;\)/
    \ contained transparent contains=ledgerAccount,ledgerMetadata,ledgerPrice
...
syn match ledgerPrice /\(\$.*\|\d\+\s\+\S\+\|\d\+\)/ contained
...
highlight default link ledgerTag Error
highlight default link ledgerTagPop Include
...
highlight default link ledgerPrice Label
