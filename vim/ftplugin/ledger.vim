...
let s:rx_amount = '\('.
                ...
                \ '\s*\%([[:alpha:]¢$€£]\+\s*\)\?'.
                \ '\%(\s*;.*\)\?$'
let s:rx_account = '\(Liabilities\|Assets\):\(\(\(\S\+\| \+\)*:\)\?\(\S\+\| \+\)\+\)'
let s:rx_account = '^\s\([^;]\)*:\(\(\S\| \)\+\)'
...
function! LedgerFoldText() "{{{1
  ...
  let groups = matchlist(line, s:rx_account)
  if ! empty(groups)
    let account = groups[2]
  endif
  ...
  let columns -= s:multibyte_strlen(printf(fmt, '', account, amount))
  ...
  return printf(fmt, foldtext, account, amount)
endfunction "}}}
